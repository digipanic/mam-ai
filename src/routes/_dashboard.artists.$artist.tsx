...
function ArtistProfile() {
  ...
  return <PageFrame eyebrow="Artist profile" title={artist.name} description={description}>
    ...
    {artist.historical.soundcloud.length > 0 && <section className="mt-10"><Panel title="Recorded SoundCloud checkpoints"><p className="text-sm text-muted-foreground">Historical checkpoints through {artist.historical.soundcloud.at(-1)?.date ?? "—"}. Current SoundCloud audience remains the live monitoring observation above.</p><div className="mt-5 grid gap-3 text-sm sm:grid-cols-3"><div><p className="text-xs text-muted-foreground">First checkpoint</p><p className="mt-1 font-semibold">{formatNumber(artist.historical.soundcloud[0]?.followers)}</p></div><div><p className="text-xs text-muted-foreground">4 Sep checkpoint</p><p className="mt-1 font-semibold">{formatNumber(artist.historical.soundcloud.find((point) => point.date === "2026-09-04")?.followers)}</p></div><div><p className="text-xs text-muted-foreground">Latest checkpoint</p><p className="mt-1 font-semibold">{formatNumber(artist.historical.soundcloud.at(-1)?.followers)}</p></div></div></Panel></section>}
  </PageFrame>;
}
