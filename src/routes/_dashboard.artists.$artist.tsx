...
function ArtistProfile() {
  ...
  return <PageFrame eyebrow="Artist profile" title={artist.name} description={description}>
    ...
    <section className="mt-10"><Panel title="Recorded monthly Instagram"><MonthlyLine artists={[artist]} /><p className="mt-3 text-xs text-muted-foreground">Historical checkpoints are shown separately from current monitoring observations.</p></Panel></section>
    ...
    {(artist.historical.instagram.length > 0 || artist.historical.soundcloud.length > 0) && <section className="mt-10"><div className="mb-6"><p className="text-[10px] uppercase tracking-[.12em] text-muted-foreground">Historical audience</p><h2 className="mt-2 font-serif text-3xl">Viberate checkpoints</h2><p className="mt-2 text-sm text-muted-foreground">These recorded historical checkpoints are separate from the current live monitoring figures above.</p></div><div className="grid gap-10 xl:grid-cols-2">{artist.historical.instagram.length > 0 && <HistoricalCheckpointPanel platform="Instagram" history={artist.historical.instagram} performance={instagramHistorical}/>} {artist.historical.soundcloud.length > 0 && <HistoricalCheckpointPanel platform="SoundCloud" history={artist.historical.soundcloud} performance={soundcloudHistorical}/>}</div></section>}
  </PageFrame>;
}
