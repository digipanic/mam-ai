...
import { dashboardSummary, findArtist, formatDate, formatNumber, formatPercent, formatSigned, historicalPerformance, monthlyConsistency, monthlyPerformance, percentile, recordHigh, trajectory } from "@/lib/audience-selectors";
...
function ArtistProfile() {
  ...
  const platforms = [
    { label: "Instagram", value: artist.instagram.audience, date: artist.instagram.observedAt, source: artist.instagram.source },
    { label: "SoundCloud", value: artist.soundcloud.audience, date: artist.soundcloud.observedAt, source: artist.soundcloud.source },
    { label: "Resident Advisor", value: artist.residentAdvisor.audience, date: artist.residentAdvisor.observedAt, source: artist.residentAdvisor.source },
  ];
  const rankingSets = [
  ...
  const instagramHistorical = historicalPerformance(artist.historical.instagram);
  const soundcloudHistorical = historicalPerformance(artist.historical.soundcloud);
  ...
  const profileDetails = [["Location", artist.location], ["Role", artist.role], ["Instagram handle", artist.instagramHandle ? `@${artist.instagramHandle.replace(/^@/, "")}` : null], ["Instagram baseline", formatNumber(artist.instagram.baseline)], ["Baseline observed", formatDate(artist.instagram.baselineAt)], ["Latest sheet observation", formatDate(data.latestSourceObservation, true)], ["Dashboard refreshed", formatDate(data.refreshedAt, true)]];
...
function HistoricalCheckpointPanel({ platform, history, performance }: { platform: "Instagram" | "SoundCloud"; history: import("@/lib/audience-types").HistoricalPoint[]; performance: ReturnType<typeof historicalPerformance> }) {
  const source = history[0]?.source ?? "Viberate";
  return <Panel title={`${platform} historical checkpoints`}><p className="text-sm text-muted-foreground">{source} historical checkpoints · {formatDate(history[0]?.date)} → {formatDate(history.at(-1)?.date)}</p><div className="mt-5 grid grid-cols-2 gap-x-6 gap-y-5 text-sm sm:grid-cols-4"><HistoricalStat label="First checkpoint" value={formatNumber(history[0]?.followers)}/><HistoricalStat label="Latest checkpoint" value={formatNumber(history.at(-1)?.followers)}/><HistoricalStat label="Follower change" value={formatSigned(performance?.change)}/><HistoricalStat label="Growth" value={formatPercent(performance?.growthPercent)}/></div><div className="mt-7"><HistoricalCheckpointLine history={history} platform={platform}/></div></Panel>;
}
