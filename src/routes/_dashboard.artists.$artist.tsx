...
import { dashboardSummary, findArtist, formatDate, formatNumber, formatPercent, formatSigned, trajectory } from "@/lib/audience-selectors";

export const Route = createFileRoute("/_dashboard/artists/$artist")({
  head: () => ({ meta: [{ title: "Artist profile | Minor AM" }, { name: "description", content: "Live Minor AM artist audience profile." }, { property: "og:title", content: "Artist profile | Minor AM" }, { property: "og:description", content: "Live Minor AM artist audience profile." }, { property: "og:type", content: "website" }, { name: "twitter:card", content: "summary" }] }),
...
function ArtistProfile() {
  ...
  return <PageFrame eyebrow="Artist profile" title={artist.name} description={description}>
    ...
    <section className="mt-10 grid gap-10 xl:grid-cols-[1.1fr_.9fr]"><Panel title="Current Instagram movement"><div className="grid grid-cols-2 gap-6"><div><p className="text-xs text-muted-foreground">Follower change</p><p className="mt-2 text-2xl font-semibold">{formatSigned(artist.instagram.change)}</p></div><div><p className="text-xs text-muted-foreground">Growth</p><p className="mt-2 text-2xl font-semibold">{formatPercent(artist.instagram.growthPercent)}</p></div></div><p className="mt-5 text-xs text-muted-foreground">{formatDate(artist.instagram.baselineAt)} → {formatDate(artist.instagram.observedAt)}</p></Panel><Panel title="Roster rankings"><div className="space-y-2">{rankingSets.map(([label, roster]) => <div className="flex justify-between border-b border-border py-2 text-sm" key={label}><span>{label}</span><b>{roster.findIndex((entry) => entry.name === artist.name) + 1 || "—"}</b></div>)}</div></Panel></section>
